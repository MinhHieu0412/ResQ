...
class _DocumentaryDetailPageState extends State<DocumentaryDetailPage> {
  ...
  void _loadInitialData() {
    ...
    frontImageUrl = frontImagePath != null
        ? '$baseUrl${stripAdminPrefix(frontImagePath)}'
        : null;

    backImageUrl = backImagePath != null
    ...
    try {
      ...
      setState(() {
        ...
        if (match != null) {
          ...
          if (typeController.text.isEmpty || typeController.text.startsWith("Vehicle Registration")) {
            typeController.text = "Vehicle Registration - ${match['plateNo'] ?? ''}";
          }
        }
      });
    } catch (e) {
  ...
  Future<void> _handleUpdate() async {
    ...
    final expirationDate = _toBackendFormat(expirationDateController.text.trim());
      ...
      children: [
            ...
            child: Container(
              ...
              child: imageFile != null
                ...
                child: Image.network(imageUrl, fit: BoxFit.cover),
                  ...
                  : Center(
                child: Icon(Icons.add_circle_outline, color: Color(0xFF013171), size: 40),
              ),
            ),
          ),
        ),
        if (_errors[fieldName] != null)
          Padding(
            ...
            child: Text(_errors[fieldName]!, style: const TextStyle(color: Colors.red, fontSize: 12)),
          ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text("Document Detail", style: TextStyle(fontSize: 24, fontWeight: FontWeight.w700, color: Colors.white)),
        leading: IconButton(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInput(typeController, fieldName: 'documentType', readOnly: true),
            const SizedBox(height: 20),
            const Text("Vehicle:", style: _labelStyle),
            const SizedBox(height: 6),
            vehicles.isEmpty
                ? const Text("Loading vehicles...", style: TextStyle(color: Colors.grey))
              ...
              onChanged: isEditing
                  ? (value) {
                final selected = vehicles.firstWhere((v) => v['vehicleId'] == value);
                setState(() {
                  selectedVehicleId = value;
                  typeController.text = "Vehicle Registration - ${selected['plateNo'] ?? ''}";
                });
              }
                  : null,
              decoration: InputDecoration(
            ...
            const SizedBox(height: 6),
            _buildInput(documentNoController, fieldName: "documentNumber", readOnly: !isEditing),
            const SizedBox(height: 20),
            ...
            Row(
              children: [
                Expanded(child: _buildImagePicker(1, 'frontImage', _frontImageFile)),
                const SizedBox(width: 16),
                Expanded(child: _buildImagePicker(2, 'backImage', _backImageFile)),
              ],
            ),
