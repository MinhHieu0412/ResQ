...
import { reqResQsAPI, extraSrvAPI, feedbackAPI } from "../../../../admin";
...
const History = ({ customer }) => {
  ...
  const fetchHistory = async () => {
    try {
      const response = await reqResQsAPI.findReqResQsByUser(customer.userid);
  ...
  const searchHistory = async () => {
    try {
      if (keyword.trim() == "") {
        ...
        const response = await reqResQsAPI.searchCustomer(customer.userid, keyword);
        setHistory(response.data);
        setIsLoading(false);
      }
    } catch (err) {
  ...
  const filteredHistory = history
    .filter((his) => {
      const statusMatch = statusFilter ? his.reqStatus === statusFilter : true;
      const serviceMatch = serviceFilter ? his.rescueType === serviceFilter : true;
  ...
  const handleViewDetail = async (his) => {
    setDetail(his);
    try {
      const response = await reqResQsAPI.relatedRecordCheck(his.rrid);
    ...
    try {
      const response = await extraSrvAPI.findExtrasByResResQ(his.rrid);
  ...
  const handleViewFeedback = async (his) => {
    try {
      const response = await feedbackAPI.findFeedbacksByReqResQ(his.rrid);
    ...
    <div>
          ...
          <tr>
            ...
            <th>
              ...
              <button onClick={() => toggleSort("date")}>
                <img
                  src={`/images/icon-web/Chevron ${sortField === "date" && sortOrder === "asc" ? "Up" : "Down"}.png`}
                  className="h-3 ml-2"
                  alt="Sort date"
                />
              </button>
      ...
      {detail && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white w-[1000px] rounded-xl p-8 relative">
